...
In the second stage, a visual foundation segmentation model (SAM 3) is applied to correct those buildings identified as potentially improvable. Unlike the MLLM, the segmentation model excels at precise geometric delineation but lacks higher-level reasoning and contextual understanding. Therefore, the MLLM not only performs validation but also generates spatial prompts that guide the segmentation process.
...
For this discovery step, a text-based prompt such as ``house'' or ``building'' is used. The multimodal prompting capability of SAM 3 allows the model to use semantic information in addition to spatial prompts.
