...
\begin{itemize}
    ...
    %TODO any better names for clean and debug patch? so it is easier to relate its name to its usage? e.g. segmented patch, promptable patch???
...
%TODOsince this is the key step of the pipeline, explain it more: for example, show example of the text prompts for 3 reasoning tasks.you can just show some texts used in your codes
...
To avoid this limitation, the system expands the spatial context around the building. %TODOhow? for example, what is the defined parameters for expansion, e.g. expand the window x4?
A larger image window is extracted that contains additional surrounding structures. This extended context enables the pipeline to detect additional building segments and to reconstruct the full building extent before performing the geometric refinement.
...
\centering
\includegraphics[width=\linewidth]{Franzelin_Master/img/Flowchart_SAMStage_IncreaseFont.png}
