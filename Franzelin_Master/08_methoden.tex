...
For each building, the mean brightness inside the building mask and inside the surrounding ring was calculated. The RGB values were first converted to linearized luminance so that the brightness comparison was not based directly on gamma-encoded image values. The final contrast value used in the results is the absolute Weber contrast:

\[
C = \left|\frac{Y_\mathrm{building} - Y_\mathrm{surrounding}}{Y_\mathrm{surrounding}}\right| .
\]
